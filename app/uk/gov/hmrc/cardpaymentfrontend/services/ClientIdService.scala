...
class ClientIdService @Inject() {
  ...
  def determineClientId(journey: Journey[?], language: Language): ClientId = {
    journey.origin match {
      ...
      case BtaVat | VcVatReturn | VcVatOther | `3psVat` | DdVat =>
        language match {
      ...
      case PfCt | BtaCt | WcCt =>
        language match {
          case Languages.English => ClientIds.COEE
          case Languages.Welsh   => ClientIds.COEC
        }

      case PfEpayeNi | PfEpayeP11d | WcEpayeNi | WcClass1aNi | BtaEpayeBill | BtaEpayeGeneral | BtaClass1aNi =>
        language match {
          case Languages.English => ClientIds.PAEE
          case Languages.Welsh   => ClientIds.PAEC
        }

      case Amls | AppSimpleAssessment | BtaEpayePenalty | BtaEpayeInterest | PfAmls | PfEpayeLpp | PfEpayeSeta | PfEpayeLateCis | WcEpayeLateCis |
          PfJobRetentionScheme | JrsJobRetentionScheme | PfOther | PfPsAdmin | BtaSdil | PfMgd | PfGamingOrBingoDuty | PfGbPbRgDuty | PfSdil |
          PfSimpleAssessment | WcSimpleAssessment | PfTpes | PfPpt | PfTrust | EconomicCrimeLevy | PfEconomicCrimeLevy | WcXref | WcEpayeLpp | WcEpayeSeta |
          DdSdil =>
        language match {
          case Languages.English => ClientIds.MIEE
          case Languages.Welsh   => ClientIds.MIEC
      ...
      case PfP800 | PtaP800 | Ppt | PtaSimpleAssessment | CapitalGainsTax | AlcoholDuty | PfAlcoholDuty | StampTaxesOnShares =>
        language match {
      ...
      case o @ (PfClass2Ni | PfInsurancePremium | Parcels | PfSpiritDrinks | PfInheritanceTax | PfWineAndCider | PfBioFuels | PfAirPass | PfBeerDuty |
          PfLandfillTax | PfAggregatesLevy | PfClimateChangeLevy | PfImportedVehicles | PfAted | PfPillar2 | Pillar2 | WcClass2Ni) =>
        throw new MatchError(s"Trying to find a client id for an unsupported origin: ${o.entryName}")
