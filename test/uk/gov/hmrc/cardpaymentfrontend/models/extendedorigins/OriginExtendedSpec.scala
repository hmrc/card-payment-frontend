  ...
  "implicit class OriginExtended" - {
    "isAWebChatOrigin should return" - {
      val originsSeq = Seq[(Origin, Boolean)](
        ...
        PfTrust                  -> false,
        PtaClass3Ni              -> false,
        PtaCheckYourStatePension -> false,
        `3psSa`                  -> false,
        `3psVat`                 -> false,
        PfPillar2                -> false,
        Pillar2                  -> false,
      ...
      "all origins should be covered" in {
        TestHelpers.implementedOrigins ++ TestHelpers.unimplementedOrigins should contain theSameElementsAs originsSeq.map(
          _._1
        ) withClue "** add new origin to seq above **"
      }
    }
