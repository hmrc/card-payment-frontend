  ...
  "PaymentCompleteController" - {
    ...
    "GET /payment-complete" - {
      ...
      "should have a test for all origins below this one" in {
        TestHelpers.implementedOrigins.size shouldBe 42 withClue "** This dummy test is here to remind you to update the tests below. Bump up the expected number when an origin is added to implemented origins **"
      }
