...
class ExternalNavigationSpec extends UnitSpec with TableDrivenPropertyChecks {
  ...
  "returnUrlCancelled" - {
    ...
    val scenarios: TableFor2[JourneyStatuses[_ >: JsdPfSa with JsdPfCt with JsdPfEpayeP11d with JsdBtaCt with JsdPfEpayeLateCis with JsdPfEpayeLpp with JsdBtaSa with JsdAlcoholDuty with JsdItSa with JsdPfEpayeSeta with JsdPtaSa with JsdPfAlcoholDuty with JsdPfEpayeNi with JsdPfVat with JsdBtaVat with JsdVcVatReturn with JsdVcVatOther <: JourneySpecificData], Option[Url]] = Table(
      ("journey", "expectedUrl"),
