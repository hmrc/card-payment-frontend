...
class ExternalNavigationSpec extends UnitSpec with TableDrivenPropertyChecks {
  ...
  "returnUrlCancelled" - {
    ...
    val scenarios: TableFor2[JourneyStatuses[_ >: JsdBounds <: JourneySpecificData], Option[Url]] = Table(
      ...
      (TestJourneys.PfTpes, None),
      (TestJourneys.PfMgd, None),
      (TestJourneys.PfGbPbRgDuty, None),
      (TestJourneys.PfTrust, None),
      (TestJourneys.PfOther, None),
      (TestJourneys.PfPsAdmin, None),
